...
function tokenizeTitle(title: string) {
  return new Set(
    normalizeTitle(title)
      ...
      .filter((token) => !["the", "for", "and", "with", "cincinnati"].includes(token))
  );
}
...
export function getDuplicateConfidence(left: ScoutEvent, right: ScoutEvent) {
  ...
  const overlapBase = Math.min(leftTokens.size, rightTokens.size) || 1;
  const titleSimilarity = Math.max(intersection / union, intersection / overlapBase);
...
export function isLikelyDuplicate(left: ScoutEvent, right: ScoutEvent) {
  ...
  const sameDay = left.startDateTime.slice(0, 10) === right.startDateTime.slice(0, 10);
  return sameDay && getDuplicateConfidence(left, right) >= 0.72;
}
